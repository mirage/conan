...
let parse_disp s =
  if is_prefix ~affix:lparent s && is_suffix ~affix:rparent s then
    Integer.parse ((tail <.> tail ~rev:true) s) >|= fun (v, s) -> (`Ind v, s)
  else Integer.parse s >|= fun (v, s) -> (`Dir v, s)

let parse_abs_or_rel s =
  let rel, s =
    if is_prefix ~affix:ampersand s then (true, tail s) else (false, s)
  in
...
let parse_indirect_offset s =
  if is_prefix ~affix:lparent s && is_suffix ~affix:rparent s then
    ...
    let arithmetic, s =
      let invert, s =
        if is_prefix ~affix:tilde s then (true, tail s) else (false, s)
      in
      let arithmetic, s = span ~min:1 ~max:1 ~sat:Arithmetic.is s in
      if length arithmetic = 1 then (Some (invert, to_string arithmetic), s)
      else (None, s)
    in
    ...
    | Some (invert, arithmetic) ->
        ...
        if is_empty empty then
          ...
          let arithmetic =
            if invert then Arithmetic.invert arithmetic else arithmetic
          in
...
let parse_offset s =
  ...
  if is_prefix ~affix:lparent ss then
    ...
    if is_empty empty then Ok (length level, offset)
...
let parse_type s =
  let unsigned, s =
    if is_prefix ~affix:u s then (true, tail s) else (false, s)
  in
  ...
  let res =
    ...
    | "stri" ->
        ...
        if string16 then prefix ~affix:_16 s >|= fun s -> (`String16, s)
        else Ok (`String, s)
  ...
  | #regex -> (
      ...
      | Some (empty, s) ->
          if is_empty empty then
            ...
            | Some (a, b) ->
                let limit, flags =
                  if for_all is_digit a then (a, b) else (b, a)
                in
  ...
  | #numeric as numeric -> (
      let arithmetic, s =
        let arithmetic, s = span ~min:1 ~max:1 ~sat:Arithmetic.is s in
        if length arithmetic = 1 then (Some (to_string arithmetic), s)
        else (None, s)
      in
      match arithmetic with
      | Some arithmetic ->
...
let parse_test s =
  if is_x s then Ok `True
    ...
    let comparison =
      if is_empty comparison then "=" else to_string comparison
    in
    let s = trim s in
