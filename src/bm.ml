...
let memcmp v a b l =
  let pos = ref 0 in
  while !pos < l && v.[a + !pos] = v.[b + !pos] do
    incr pos
  done ;
...
let boyer_moore_needle_match needle nlen portion offset =
  ...
  then false
  else memcmp needle (nlen - portion + !ignore) !va (portion - !ignore)
...
let skip needle nlen =
  ...
  for a = 0 to nlen - 1 do
    let value = ref 0 in
    while
      !value < nlen && not (boyer_moore_needle_match needle nlen a !value)
    do
      incr value
    done ;
    skip.(nlen - a - 1) <- !value
...
let search { bind; return } ~get text hpos occ skip needle nlen =
  let ( >>= ) x f =
    bind x (function Ok x -> f x | Error err -> return (Error err)) in
  ...
  fun ({ bind; return } as s) ~ln ~get text ->
    ...
    let rec go hpos acc =
      ...
      then return (Ok acc)
        ...
        search s ~get text hpos occ skip needle nlen >>= function
        | -1L, hpos -> go hpos acc
