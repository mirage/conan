...
let process
  ...
  = fun ({ bind; return; } as scheduler) syscall fd offset abs_offset ->
    ...
    and go_calculation abs_offset = function
      ...
      | Arithmetic.Add v ->
        go_offset v >?= fun v ->
        (return <.> ok) (Int64.add abs_offset v)
      | Arithmetic.Sub v ->
        go_offset v >?= fun v ->
        (return <.> ok) (Int64.sub abs_offset v)
      | Arithmetic.Mul v ->
        go_offset v >?= fun v ->
        (return <.> ok) (Int64.mul abs_offset v)
      | Arithmetic.Div v ->
        go_offset v >?= fun v ->
        (return <.> ok) (Int64.div abs_offset v)
      | Arithmetic.Mod v ->
        go_offset v >?= fun v ->
        (return <.> ok) (Int64.rem abs_offset v)
