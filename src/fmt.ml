...
module Hmap = struct
  ...
  module type Tid = sig type t type _ Tid.t += Tid : t Tid.t end
  ...
  type binding = B : 'a key * (?padding:[ `Left of int | `Right of int | `Zero of int ] -> ?precision:int -> formatter -> 'a -> unit) -> binding
  ...
  let find : type a. a key -> t -> (?padding:[ `Left of int | `Right of int | `Zero of int ] -> ?precision:int -> Format.formatter -> a -> unit) option =
    fun k t -> match Map.find (Key.V k) t with
      | B (k', v) ->
  ...
  let get k t = match find k t with
    | Some pp -> pp
...
type ('ty, 'v) order =
  | Byte  : ('u, 'v) padding *  ('v, char -> 'w) precision -> ('u, 'w) order
  | Short : ('u, 'v) padding *   ('v, int -> 'w) precision -> ('u, 'w) order
  | Long  : ('u, 'v) padding * ('v, int32 -> 'w) precision -> ('u, 'w) order
  | Quad  : ('u, 'v) padding * ('v, int64 -> 'w) precision -> ('u, 'w) order
...
type ('v, 'r) ppw = V : ('a, 'b) padding * ('b, 'c) precision * ('c, 'd) ty -> ('a, 'd) ppw
...
let pp_ty : type v r. Format.formatter -> (v, r) ty -> unit = fun ppf ty ->
  let rec flat : type v r. _ list -> (v, r) ty -> _ list = fun acc -> function
    | End -> List.rev acc
...
let gen_padding_precision
  : type u v w ty0 v0. (u, v) padding -> (v, w) precision -> (ty0, v0) ty -> (ty0, v0) ppw
  = fun padding precision ty -> match precision, gen_padding padding ty with
...
let rec gen
  : type ty0 v0 ty1 v1. (ty0, v0) fmt -> (ty1, v1) ty -> (ty1, v1) tw
  = fun fmt ty -> match fmt, ty with
    | [], rest -> T ([], rest)
    ...
    | Const (pp, v) :: fmt_rest, ty_rest ->
      let T (fmt, ty) = gen fmt_rest ty_rest in
      T (Const (pp, v) :: fmt, ty)
    | Byte (padding, precision) :: fmt_rest, _ ->
      ( match gen_padding_precision padding precision ty with
        | V (padding, precision, Char ty_rest) ->
          let T (fmt, ty) = gen fmt_rest ty_rest in
          T (Byte (padding, precision) :: fmt, ty)
        | _ -> raise Invalid_type )
    | Short (padding, precision) :: fmt_rest, _ ->
      ( match gen_padding_precision padding precision ty with
        | V (padding, precision, Int ty_rest) ->
          let T (fmt, ty) = gen fmt_rest ty_rest in
          T (Short (padding, precision) :: fmt, ty)
        | _ -> raise Invalid_type )
    | Long (padding, precision) :: fmt_rest, _ ->
      ( match gen_padding_precision padding precision ty with
        | V (padding, precision, Int32 ty_rest) ->
          let T (fmt, ty) = gen fmt_rest ty_rest in
          T (Long (padding, precision) :: fmt, ty)
        | _ -> raise Invalid_type )
    | Quad (padding, precision) :: fmt_rest, _ ->
      ( match gen_padding_precision padding precision ty with
        | V (padding, precision, Int64 ty_rest) ->
          let T (fmt, ty) = gen fmt_rest ty_rest in
          T (Quad (padding, precision) :: fmt, ty)
        | _ -> raise Invalid_type )
      ...
      ( match gen_padding_precision padding precision ty with
        | V (padding, precision, Any (key1, ty_rest)) ->
          ( if not Hmap.Key.(equal (hide_type key0) (hide_type key1))
            ...
            else match Hmap.refl key0.Hmap.Key.tid key1.Hmap.Key.tid with
              | Some Refl ->
                let T (fmt, ty) = gen fmt_rest ty_rest in
                T (Atom (padding, precision, key0) :: fmt, ty)
              | None -> raise Invalid_key )
        | _ -> raise Invalid_type )
    | Noop :: fmt_rest, ty_rest ->
      gen fmt_rest ty_rest
...
let rec concat
  : type u v w. (u, v) fmt -> (v, w) fmt -> (u, w) fmt
  = fun l1 l2 -> match l1 with
    | [] -> l2
    | x :: r -> x :: concat r l2 (* XXX(dinosaure): tail-rec? *)
...
type ('x, 'r) pdprebb = PdPrEbb : ('u, 'v) padding * ('v, 'w -> 'a) precision * ('a, 'r) fmt -> ('w, 'r) pdprebb

let make_prebb
  : type a b. (a, b) precision -> (_, _) fmt -> (_, _) prebb
  = fun precision fmt -> match precision with
    | Nop -> PrEbb (Nop, fmt)
    | Lit v -> PrEbb (Lit v, fmt)
    | Arg -> PrEbb (Arg, fmt)

let make_pdprebb
  : type a b c d. (a, b) padding -> (c, d) precision -> (_, _) fmt -> (_, _) pdprebb
...
let rec parse_precision
  ...
  then let precision, s = span ~sat:is_digit (tail s) in
    ...
    | precision -> parse_flag padding ~any (Lit (int_of_string precision)) s
  else parse_flag ~any padding Nop s
...
and parse_padding
  ...
  | false, false, pad -> parse_precision ~any (Lit (`Right, int_of_string pad)) s
  | true,  false, pad -> parse_precision ~any (Lit (`Left, int_of_string pad)) s
  | false, true,  pad -> parse_precision ~any (Lit (`Zero, int_of_string pad)) s
...
and parse_flag
  ...
  | Some 'c' ->
    let Ebb fmt = go ~any (tail s) in
    let PdPrEbb (padding, precision, fmt) = make_pdprebb padding precision fmt in
...
and go : type x r. any:x Hmap.key -> s -> r ebb = fun ~any s ->
  ...
  | None ->
    if is_empty s
    then Ebb []
    else Ebb [ Const (pp_string, to_string s) ]
  | Some (x, s) ->
    if not (is_empty x)
    then
...
let coerce
  ...
  = fun fmt ty -> match gen fmt ty with
  | T (fmt, End) -> fmt
  | _ -> invalid_arg "coerce"

let of_string : type x v r. any:x Hmap.key -> string -> (v, r) ty -> (v, r) fmt = fun ~any s ty ->
  let Ebb fmt = parse ~any s in
  coerce fmt ty

let ty_of_string : type x. any:x Hmap.key -> string -> w = fun ~any s ->
  let Ebb fmt = parse ~any s in
  Ty (ty_of_fmt fmt)
...
let keval_padding
  : type ty v. formatter -> (ty, v) padding
    -> (formatter -> [ `Left of int | `Right of int | `Zero of int ] option -> v) -> ty
  = fun ppf padding k -> match padding with
    | Nop -> k ppf None
...
let keval_precision
  : type ty v. formatter -> (ty, v) precision
    -> (formatter -> int option -> v) -> ty
  = fun ppf precision k -> match precision with
    | Nop -> k ppf None
    | Lit v -> k ppf (Some v)
...
let keval_order
  ...
  = fun pps ppf order k -> match order with
    | Ignore -> fun _ -> k ppf
    | Noop -> k ppf
    | Const (pp, v) -> pp ppf v ; k ppf
    | Param -> fun pp v -> pp ppf v ; k ppf
    | Atom (padding, precision, key) ->
      let k padding ppf precision v =
        let pp = Hmap.get key pps in
        pp ?padding ?precision ppf v ; k ppf in
    ...
    | Byte (padding, precision) ->
      let k padding ppf precision v = pp_char ?padding ?precision ppf v ; k ppf in
      let k ppf padding = keval_precision ppf precision (k padding) in
      keval_padding ppf padding k
    | Short (padding, precision) ->
      let k padding ppf precision v = pp_int ?padding ?precision ppf v ; k ppf in
      let k ppf padding = keval_precision ppf precision (k padding) in
      keval_padding ppf padding k
    | Long (padding, precision) ->
      let k padding ppf precision v = pp_int32 ?padding ?precision ppf v ; k ppf in
      let k ppf padding = keval_precision ppf precision (k padding) in
      keval_padding ppf padding k
    | Quad (padding, precision) ->
      let k padding ppf precision v = pp_int64 ?padding ?precision ppf v ; k ppf in
      let k ppf padding = keval_precision ppf precision (k padding) in
      keval_padding ppf padding k
    | Float (padding, precision) ->
      let k padding ppf precision v = pp_float ?padding ?precision ppf v ; k ppf in
      let k ppf padding = keval_precision ppf precision (k padding) in
      keval_padding ppf padding k

let rec keval
  : type ty v. Hmap.t -> formatter -> (ty, v) fmt -> (formatter -> v) -> ty
  = fun pps formatter fmt k -> match fmt with
    | [] -> k formatter
    | x :: r ->
