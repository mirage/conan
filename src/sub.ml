...
let fspan ~min ~max ~sat ({ s; start; stop } as sub) =
  if min < 0
  then invalid_arg "fspan"
  else if max < 0
  then invalid_arg "fspan"
  else if min > max || max = 0
  then ({ s; start; stop = start }, sub)
  else
    let max_idx = stop - 1 in
    let max_idx =
      let k = start + max - 1 in
      if k > max_idx || k < 0 then max_idx else k in
    ...
    let rec loop i =
      ...
      then (sub, { s; start = stop; stop })
      else ({ s; start; stop = i }, { s; start = i; stop }) in
    loop start

let rspan ~min ~max ~sat ({ s; start; stop } as sub) =
  if min < 0
  then invalid_arg "rspan"
  else if max < 0
  then invalid_arg "rspan"
  else if min > max || max = 0
  then (sub, { s; start = stop; stop })
    ...
    let min_idx =
      let k = stop - max in
      if k < start then start else k in
    ...
    let rec loop i =
      ...
      then ({ s; start; stop = start }, sub)
      else ({ s; start; stop = i + 1 }, { s; start = i + 1; stop }) in
    loop max_idx
        ...
        then l
          ...
          | [] ->
              ...
              then l
                ...
                then -1
                else (sep_count * sep_len) + l in
      let cat_len = cat_len 0 sub_len ss in
      if cat_len < 0
      then invalid_arg "concat"
      else
...
let trim ?(drop = is_white) ({ s; start; stop } as sub) =
  ...
  then sub
    ...
    if left = max_pos
    then { s; start = (start + stop) / 2; stop = (start + stop) / 2 }
    else
...
let with_range ?(first = 0) ?(len = max_int) { s; start; stop } =
  ...
  then invalid_arg "with_range"
    ...
    then empty first
      ...
      then empty first
      else { s; start = start + first; stop = start + last + 1 (* position *) }
...
let add_sub ~no_empty s ~start ~stop acc =
  if start = stop
  then if no_empty then acc else { s; start; stop = start } :: acc
